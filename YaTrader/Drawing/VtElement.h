...
public:
	...
	SmPoint StartValue() const { return _StartValue; }
	void StartValue(SmPoint val) { _StartValue = val; }
	SmPoint EndValue() const { return _EndValue; }
	void EndValue(SmPoint val) { _EndValue = val; }
	/// <summary>
	/// 시작점의 포인터를 가져온다.
	/// </summary>
	/// <returns></returns>
	SmPoint* GetStartValuePointer() {
		return &_StartValue;
	}
	/// <summary>
	/// 끝점의 포인터를 가져온다.
...
public:
	...
	/// <summary>
	/// 현재 있는 값을 중심으로 움직임을 시작할 수 있는 화면 좌표를 설정한다.
	/// 이 좌표를 중심으로 화면 객체를 움직이거나 모양을 바꿀 수 있다.
	/// </summary>
	virtual void SetMovingPoint(DoubleArray& timeStamps, XYChart* chart);
...
protected:
	...
	// 객체의 시작 값 - 실제 차트 상의 값이다. - 그릴 때는 이 값을 화면 좌표로 바꾸어 그려줘야 한다.
	SmPoint _StartValue;
	// 객체의 종료 값 - 실제 차트 상의 값이다. 그릴 때는 이 값을 화면 좌표로 바꾸어 그려줘야 한다.
	SmPoint _EndValue;
};
