...
void VtShapesCollection::SanitizeRect(VtPoint& start, VtPoint& end)
{
...
void VtShapesCollection::AddRect(SmPoint start, SmPoint end, VtColor penC, int penWidth, VtColor fillC, bool filled)
{
	VtRect* rect = new VtRect(start, end, penC, penWidth, fillC, filled);
	List.push_back(rect);
}
...
void VtShapesCollection::AddLine(SmPoint start, SmPoint end, VtColor penColor, int penWidth)
{
	VtLine* line = new VtLine(start, end, penColor, penWidth);
	List.push_back(line);
}
...
void VtShapesCollection::AddEllipse(SmPoint start, SmPoint end, VtColor penC, int penWidth, VtColor fillC, bool filled)
{
	VtEllipse* rect = new VtEllipse(start, end, penC, penWidth, fillC, filled);
	List.push_back(rect);
}
...
void VtShapesCollection::DrawObjects(DoubleArray& timeStamp, XYChart* chart)
{
	for (auto i = List.begin(); i != List.end(); i++)
	{
		if (!(*i)->IsSelected())
		{
			(*i)->Draw(timeStamp, chart);
		}
