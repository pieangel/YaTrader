...
void VtElement::SetMovingPoint(DoubleArray& timeStamps, XYChart* chart)
{
	int xIndex = (int)floor(Chart::bSearch(timeStamps, _StartValue.x));
	Start0.x = chart->getXCoor(xIndex);
	Start0.y = chart->getYCoor(_StartValue.y);
	int xIndex2 = (int)floor(Chart::bSearch(timeStamps, _EndValue.x));
	Start1.x = chart->getXCoor(xIndex2);
	Start1.y = chart->getYCoor(_EndValue.y);
}
