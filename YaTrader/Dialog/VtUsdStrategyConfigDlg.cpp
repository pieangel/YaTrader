...
IMPLEMENT_DYNAMIC(VtUsdStrategyConfigDlg, CBCGPDialog)

VtUsdStrategyConfigDlg::VtUsdStrategyConfigDlg(CWnd* pParent /*=nullptr*/)
	: CBCGPDialog(IDD_SYSTEM_USD, pParent)
{
	InitComboMap();
	_ShowArgGrid = true;
	_SelSymbol = nullptr;
...
BEGIN_MESSAGE_MAP(VtUsdStrategyConfigDlg, CBCGPDialog)
	...
	ON_STN_CLICKED(IDC_STATIC_ENT_CNT_TODAY, &VtUsdStrategyConfigDlg::OnStnClickedStaticEntCntToday)
END_MESSAGE_MAP()
...
// 	시스템 아이디가 같지 않으면 업데이트 하지 않는다.
// 		if (order->SystemID != _System->Id())
// 			return;
// 	
...
// 	
// 					CString profit;
// 					profit.Format(_T("%.0f"), posi->OpenProfitLoss);
...
// 		_System->SymbolCode(_SelSymbol->ShortCode);
// 		// 시세만 등록해 준다.
// 		realtimeRegiMgr->RegisterProduct(_SelSymbol->ShortCode, 1);
// 	}
// 	
// 	CTime esTime;
...
// 	ts.TrailingPercent = _ttof(value);1
// 
...
{
// 	if (_System && _System->LogVector.size() > 0) {
...
void VtUsdStrategyConfigDlg::OnStnClickedStaticEntCntToday()
{
