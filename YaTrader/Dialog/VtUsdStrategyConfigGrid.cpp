...
void VtUsdStrategyConfigGrid::SetArg(const DarkHorse::GroupArg& argGrp)
{
	if (argGrp.name.compare(_T("매수청산")) == 0) {
...
void VtUsdStrategyConfigGrid::RefreshRealTimeValue(std::string argName, CString value)
{
	...
	if (it != _ArgMap.end()) {
		...
		if (_System /*&& cell.GetNumber() == 1.0*/) {
			...
			//QuickSetBackColor(col - 1, row, _System->GetConwndition(argName, param) ? RGB(255, 227, 132) : RGB(255, 255, 255));
