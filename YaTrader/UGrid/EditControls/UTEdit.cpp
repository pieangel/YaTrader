...
int CUTNumericEdit::ChangeSign(BOOL bNegative)
{
	...
	CString empty_str("");
	CString sLeftLiterals=(nPosInputData>0 ? sMask.Left(nPosInputData) : empty_str);
	CString sRightLiterals=(nPosInputData==-1 ? 
		empty_str : sMask.Mid(nPosInputData+1));
	...
	if(m_bNegativeValue)
	{
		...
		CString sLeftNegativeLiterals=(nPosInNegative>0 ? 
			m_sFormatNegative.Left(nPosInNegative) : empty_str);
		CString sRightNegativeLiterals=(nPosInNegative==-1 ? 
			empty_str : m_sFormatNegative.Mid(nPosInNegative+
	...
	else
	{
		...
		CString sLeftPositiveLiterals=(nPosInPositive>0 ? 
			m_sFormatPositive.Left(nPosInPositive) : empty_str);
		CString sRightPositiveLiterals=(nPosInPositive==-1 ? 
			empty_str : m_sFormatPositive.Mid(nPosInPositive+
			UTNUMERICVALUE_PLACEHOLDER_LENGTH));
...
void CUTCurrencyEdit::UpdateMask()
{
	...
	if(nPosCurrency<nPosValue)
	{
		...
		CString sLeftLiterals=(nPosInputData==-1 ? empty_str : sMask.Left(nPosInputData));
	...
	else
	{
		...
		CString sRightLiterals=(nPosInputData==-1 ? empty_str : sMask.Mid(nPosInputData+1));
