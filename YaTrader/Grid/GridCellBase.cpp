...
void CGridCellBase::AddOrder(std::shared_ptr<DarkHorse::SmOrder> order)
{
	if (!order)
		return;
...
int CGridCellBase::GetOrderCount()
{
	...
	for (auto it = OrderMap.begin(); it != OrderMap.end(); ++it)
		cellOrderCnt += it->second->OrderAmount;
	return cellOrderCnt;
}

void CGridCellBase::AddStopOrder(std::shared_ptr<DarkHorse::SmOrderRequest> order)
{
	if (!order)
		return;
...
void CGridCellBase::DrawOrder(CDC* pDC, CRect& rect)
{
	...
	if (OrderMap.size() > 0) {
		...
		CBrush* pTempBrush = NULL;
		CBrush OrigBrush;
		std::shared_ptr<SmOrder> order = OrderMap.begin()->second;
		...
		else if (order->PositionType == SmPositionType::Buy)
		{
