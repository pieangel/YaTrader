...
public:
	void AddOrder(std::shared_ptr<DarkHorse::SmOrder> order);
	void RemoveOrder(std::string orderNo);
	void ClearOrders();
	int GetOrderCount();

	void AddStopOrder(std::shared_ptr<DarkHorse::SmOrderRequest> order);
	void RemoveStopOrder(int reqNo);
	void ClearStopOrders();
	int GetStopOrderCount();
	std::map<std::string, std::shared_ptr<DarkHorse::SmOrder>> GetOrderMap() {
		return OrderMap;
	}
	std::map<int, std::shared_ptr<DarkHorse::SmOrderRequest>> GetStopOrderMap() {
		return StopOrderMap;
	}
	
protected:
    DWORD    m_nState;      // Cell state (selected/focus etc)
	std::map<std::string, std::shared_ptr<DarkHorse::SmOrder>> OrderMap;
	std::map<int, std::shared_ptr<DarkHorse::SmOrderRequest>> StopOrderMap;
