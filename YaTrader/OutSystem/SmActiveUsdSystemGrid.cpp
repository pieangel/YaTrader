...
#include "../OutSystem/UsdSystemDefGrid.h"
SmActiveUsdSystemGrid::SmActiveUsdSystemGrid()
{
	quote_control_ = std::make_shared<DarkHorse::QuoteControl>();
	quote_control_->set_event_handler(std::bind(&SmActiveUsdSystemGrid::on_update_quote, this));
}
...
void SmActiveUsdSystemGrid::OnSetup()
{
...
void SmActiveUsdSystemGrid::OnDClicked(int col, long row, RECT* rect, POINT* point, BOOL processed)
{

}

int SmActiveUsdSystemGrid::OnCellTypeNotify(long ID, int col, long row, long msg, long param)
{
...
void SmActiveUsdSystemGrid::OnMouseMove(int col, long row, POINT* point, UINT nFlags, BOOL processed /*= 0*/)
{
	if (_OldSelRow == row)
		return;
...
void SmActiveUsdSystemGrid::OnMouseLeaveFromMainGrid()
{
...
void SmActiveUsdSystemGrid::OnLClicked(int col, long row, int updn, RECT* rect, POINT* point, int processed)
{
	if (_ClickedRow >= 0) {
		for (int i = 0; i < _ColCount; ++i) {
...
void SmActiveUsdSystemGrid::OnRClicked(int col, long row, int updn, RECT* rect, POINT* point, int processed)
{
	if (_ClickedRow >= 0) {
		for (int i = 0; i < _ColCount; ++i) {
			QuickSetBackColor(i, _ClickedRow, RGB(255, 255, 255));
...
void SmActiveUsdSystemGrid::SetColTitle()
{
	CUGCell cell;
	LPCTSTR title[10] = { "계좌번호", "종목", "포지션", "평균단가", "현재가", "평가손익", "매도주문", "매수주문", "시스템이름", "청산" };
...
void SmActiveUsdSystemGrid::QuickRedrawCell(int col, long row)
{
	CRect rect;
...
void SmActiveUsdSystemGrid::Refresh()
{
	ClearCells();
	InitGrid();
}

void SmActiveUsdSystemGrid::RefreshOrders()
{
	auto usd_system_map = mainApp.out_system_manager()->get_active_usd_system_map();
	int i = 0;
	for (auto it = usd_system_map.begin(); it != usd_system_map.end(); ++it) {
	...
	_RowNumber = usd_system_map.size();
}

void SmActiveUsdSystemGrid::on_update_quote()
{
	CString log;
...
int SmActiveUsdSystemGrid::OnButton(long ID, int col, long row, long msg, long param)
{
	if (msg != 1)
		return -1;
	...
	auto found = usd_system_map_.find(row);
	if (found == usd_system_map_.end()) return 0;
	auto usd_system = found->second;
	if (usd_system) {
		// 시스템을 청산시킨다.
		usd_system->liq_all();
		// 시스템을 없애 버린다.
		mainApp.out_system_manager()->remove_usd_system(usd_system);
		// 연결창에서 체크를 풀고 시스템을 비활성화 시킨다.
		if (_UsdGrid) {
			_UsdGrid->ClearCheck(usd_system);
		}
		// 모든 셀을 지운다.
		ClearCells();
		// 그리드를 다시 그린다.
...
void SmActiveUsdSystemGrid::InitGrid()
{
	RefreshOrders();
}

void SmActiveUsdSystemGrid::ClearCells()
{
	CUGCell cell;
